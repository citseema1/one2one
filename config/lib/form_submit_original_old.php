...
//$transport = Swift_SendmailTransport::newInstance();
	...
	$namee=$_SESSION["base_dir"]." One to One meeing scheduler";
...
/* ----------------------  form post date end----------------------------------------------------       */	
	function MemberLogin($logindetail,$UserType){
	   ...
	   if($UserType=='admin'){
		...
		extract($data_array);
		Update('pre_conference', "allocate_table='".$allocate_table."'", "where 1 limit 1");
	...
	function EmailMessageSent($ScheduledmeetingID,$purpose){
	   ...
	   $select_1=mysql_query("select * from  pre_user");
...
$logo_admin=$row_1["logo"];
		...
		//	$short_name='GLWL_2013';
			
	$tmp_img_path='https://one2onescheduler.com/'.$short_name.'/admin/logo_image/logo_100_100/'.$logo_admin;
			  $message_image = '<img src='.$tmp_img_path.'>';
    
			 
			$select_email = Select("pre_emailsetting","subject,message,from_email","where purpose = '".$purpose."' and deleted=1  LIMIT 1");	
			if(count($select_email) && !empty($select_email[0]['message']) && !empty($select_email[0]['subject']) && !empty($select_email[0]['from_email'])){
             ...
             $MsID=$ScheduledmeetingArray[0]['meetingslots_id'];
                ...
                $MeetingSet=Select('pre_company a,pre_table b','a.name,a.country,a.email, b.participant_id',"WHERE b.id =".$TableID." AND a.id=b.company_id LIMIT 1");   
             ...
             /*  $meet_set=  mysql_query("select * from pre_table b inner join pre_company a on b.company_id=a.id where company_id='$MWTID'");
			
			$MeetingSet=  mysql_fetch_array($meet_set);*/
               $MeetingSet=Select('pre_company a,pre_table b','a.name, a.country,a.email, b.participant_id',"WHERE b.id =".$MWTID." AND a.id=b.company_id LIMIT 1");   
			...
			//"Meeting was cancelled by {canceled comp}"			
			//$canceld_by="Meeting was cancelled by".$Icompany."(".$IPN.")";
			    }
             ...
             $subject = $subject;
			   ...
			   ///////////////////////kkkkkkkk
	  define("HOST", "mail.one2onescheduler.com");
       define("PORT", 25);
       define("SMTPUSERNAME", "naf@one2onescheduler.com");
       define("SMTPPASSWORD", "naf@456");
       define("FROM_SMTPEMAIL","naf@one2onescheduler.com"); 
	     /*  define("HOST", "smtpout.secureserver.net");
      define("PORT", 80);
      define("SMTPUSERNAME", "undp@one2onescheduler.com");
      define("SMTPPASSWORD", "undp456");
      define("FROM_SMTPEMAIL","undp@one2onescheduler.com");*/
...
$mail->SetFrom( 'naf@one2onescheduler.com', $title);
//$mail->AddReplyTo( 'cilf@one2onescheduler.com',$title);
...
//$body=$message;
$mail->Subject =$subject;
$mail->SetFrom( 'demo@one2onescheduler.com', $title);
$mail->AddReplyTo( 'demo@one2onescheduler.com',$title);
$mail->AltBody = "To view the message, please use an HTML compatible email viewer!";
/*$mail->MsgHTML($body);
$mail->AddAddress($Iemail);
...
//Update('pre_scheduledmeeting','email_sent=1','where id='.$ScheduledmeetingID.' limit 1');  
	...
	//echo "<li>".$ScheduledmeetingID;die();
		if(($ScheduledmeetingID > 0)&&($purpose != '')){
			...
			if(count($select_email) && !empty($select_email[0]['message']) && !empty($select_email[0]['subject']) && !empty($select_email[0]['from_email'])){
				extract($select_email[0]);
				$ScheduledmeetingArray=Select('pre_scheduledmeeting', 'table_id, scheduled_with_table_id, meetingslots_id','where id='.$ScheduledmeetingID.' LIMIT 1');
