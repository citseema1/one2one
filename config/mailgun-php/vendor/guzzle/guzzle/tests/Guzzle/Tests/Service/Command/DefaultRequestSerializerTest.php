...
class DefaultRequestSerializerTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function setUp()
    {
        ...
        $this->client = new Client('https://foo.com/baz');
    ...
    public function testUsesRelativePath()
    {
        ...
        $request = $this->serializer->prepare($this->command);
        $this->assertEquals('https://foo.com/baz/bar', (string) $request->getUrl());
    }
    ...
    public function testUsesRelativePathWithUriLocations()
    {
        ...
        $request = $this->serializer->prepare($this->command);
        $this->assertEquals('https://foo.com/baz/bar/123', (string) $request->getUrl());
    }
    ...
    public function testMixedParams()
    {
        ...
        $request = $this->serializer->prepare($this->command);
        $this->assertEquals('https://foo.com/baz/bar?fields='.urlencode('id,name'), (string) $request->getUrl());
    }
