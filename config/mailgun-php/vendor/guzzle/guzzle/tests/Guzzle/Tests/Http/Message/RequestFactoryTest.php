...
class HttpRequestFactoryTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    {
        $request = RequestFactory::getInstance()->create('GET', 'https://www.google.com/');
        ...
        $this->assertEquals('https://www.google.com/', $request->getUrl());
    ...
    {
        // Test using a string
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/path?q=1&v=2', null, 'Data');
        $this->assertInstanceOf('Guzzle\\Http\\Message\\EntityEnclosingRequest', $request);
        $this->assertEquals('PUT', $request->getMethod());
        $this->assertEquals('http', $request->getScheme());
        $this->assertEquals('https://www.google.com/path?q=1&v=2', $request->getUrl());
        $this->assertEquals('www.google.com', $request->getHost());
        ...
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/path?q=1&v=2', null, EntityBody::factory('Data'));
        $this->assertInstanceOf('Guzzle\\Http\\Message\\EntityEnclosingRequest', $request);
        $this->assertEquals('Data', (string) $request->getBody());
        ...
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/path?q=1&v=2', null, $resource);
        $this->assertInstanceOf('Guzzle\\Http\\Message\\EntityEnclosingRequest', $request);
        $this->assertEquals('Data', (string) $request->getBody());

        // Test using an object that can be cast as a string
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/path?q=1&v=2', null, Url::factory('https://www.example.com/'));
        $this->assertInstanceOf('Guzzle\\Http\\Message\\EntityEnclosingRequest', $request);
        $this->assertEquals('https://www.example.com/', (string) $request->getBody());
    }

    public function testCreatesHeadAndDeleteRequests()
    {
        $request = RequestFactory::getInstance()->create('DELETE', 'https://www.test.com/');
        $this->assertEquals('DELETE', $request->getMethod());
        $request = RequestFactory::getInstance()->create('HEAD', 'https://www.test.com/');
        $this->assertEquals('HEAD', $request->getMethod());
    }

    public function testCreatesOptionsRequests()
    {
        $request = RequestFactory::getInstance()->create('OPTIONS', 'https://www.example.com/');
    ...
    {
        // Use an array
        $request = RequestFactory::getInstance()->create('POST', 'https://www.google.com/path?q=1&v=2', null, array(
            'a' => 'b'
        ));
        ...
        // Use a collection
        $request = RequestFactory::getInstance()->create('POST', 'https://www.google.com/path?q=1&v=2', null, new Collection(array(
            'a' => 'b'
        )));
        $this->assertEquals(array('a' => 'b'), $request->getPostFields()->getAll());

        // Use a QueryString
        $request = RequestFactory::getInstance()->create('POST', 'https://www.google.com/path?q=1&v=2', null, new QueryString(array(
            'a' => 'b'
        )));
        $this->assertEquals(array('a' => 'b'), $request->getPostFields()->getAll());

        $request = RequestFactory::getInstance()->create('POST', 'https://www.test.com/', null, array(
            'a' => 'b',
            'file' => '@' . __FILE__
        ));
    ...
    {
        $parts = parse_url('https://michael:123@www.google.com:8080/path?q=1&v=2');

        $request = RequestFactory::getInstance()->fromParts('PUT', $parts, null, 'Data');
        $this->assertInstanceOf('Guzzle\\Http\\Message\\EntityEnclosingRequest', $request);
        $this->assertEquals('PUT', $request->getMethod());
        $this->assertEquals('http', $request->getScheme());
        $this->assertEquals('https://www.google.com:8080/path?q=1&v=2', $request->getUrl());
        $this->assertEquals('www.google.com', $request->getHost());
    ...
    {
        $request = RequestFactory::getInstance()->create('TRACE', 'https://www.google.com/');
    ...
    {
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/', array(
    ...
    {
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.foo.com/', array(
            'Transfer-Encoding' => 'chunked'
        ), 'Test');
        $this->assertFalse($request->hasHeader('Content-Length'));
        $this->assertEquals('chunked', $request->getHeader('Transfer-Encoding'));

        $request = RequestFactory::getInstance()->create('POST', 'https://www.foo.com/', array(
            'transfer-encoding' => 'chunked'
        ), array(
            'foo' => 'bar'
        ));
    ...
    public function testClonesRequestsWithMethodWithoutClient()
    {
        ...
        $request = $f->create('GET', 'https://www.test.com', array('X-Foo' => 'Bar'));
        ...
        $this->assertEquals('https://www.test.com', $cloned->getUrl());
    ...
    public function testClonesRequestsWithMethodWithClient()
    {
        $f = RequestFactory::getInstance();
        $client = new Client();
        $request = $client->put('https://www.test.com', array('Content-Length' => 4), 'test');
    ...
    {
        $request = RequestFactory::getInstance()->create('GET', 'https://foo.com', array(), null, array(
            'query' => array('Foo' => 'Bar')
        ));
    ...
    {
        $request = new Request('GET', 'https://www.foo.com?test=abc');
    ...
    {
        $request = new Request('GET', 'https://www.foo.com', array('Foo' => 'Bar'));
        RequestFactory::getInstance()->applyOptions($request, array(
    ...
    {
        $request = RequestFactory::getInstance()->create('PUT', 'https://test.com', array(), '0');
