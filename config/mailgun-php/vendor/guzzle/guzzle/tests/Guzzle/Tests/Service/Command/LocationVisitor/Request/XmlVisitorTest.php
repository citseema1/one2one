...
class XmlVisitorTest extends AbstractVisitorTestCase
{
    public function xmlProvider()
    {
        return array(
            array(
                ...
                '<test xmlns="https://foo.com"><Foo>test</Foo><Baz>bar</Baz></test>'
            ),
                ...
                array(
                    'parameters' => array(
                        'Foo' => array(
                            ...
                            'type' => 'string',
                            'data' => array(
                                'xmlNamespace' => 'https://foo.com'
                            )
                        )
                    )
                ),
                array('Foo' => 'test'),
                '<Request><Foo xmlns="https://foo.com">test</Foo></Request>'
            ),
            ...
            array(
                ...
                '<Request><Wrap xsi:baz="test" xmlns:xsi="https://foo.com"/></Request>'
            ),
            ...
            array(
                ...
                '<Request><Wrap><xsi:Foo xmlns:xsi="https://foobar.com">test</xsi:Foo></Wrap></Request>'
            ),
            array(
                array(
                    'parameters' => array(
                ...
                '<Request><Foo xmlns="https://foo.com"><![CDATA[<h1>This is a title</h1>]]></Foo></Request>'
            ),
    ...
    public function testSerializesXml(array $operation, array $input, $xml)
    {
        ...
        $command->setClient(new Client('https://www.test.com/some/path.php'));
    ...
    public function testAddsContentTypeAndTopLevelValues()
    {
        ...
        $this->assertEquals(
            '<?xml version="1.0"?>' . "\n"
            . '<test xmlns:xsi="https://foo.com"><Foo>test</Foo><Baz>bar</Baz></test>' . "\n",
    ...
    {
        $request = new EntityEnclosingRequest('POST', 'https://foo.com');
        $visitor = new XmlVisitor();
    ...
    public function testCanAddMultipleNamespacesToRoot()
    {
        ...
        $this->assertEquals(
            '<?xml version="1.0"?>' . "\n"
            . '<Hi xmlns:xsi="https://foo.com" xmlns:foo="https://foobar.com"><Foo>test</Foo></Hi>' . "\n",
            (string) $request->getBody()
        );
    }
    ...
    public function testAllowsSendingXmlPayloadIfNoXmlParamsWereSet()
    {
        ...
        $command = $this->getMockBuilder('Guzzle\Service\Command\OperationCommand')
            ...
            ->getMockForAbstractClass();
        $command->setClient(new Client('https://foo.com'));
        $request = $command->prepare();
        $this->assertEquals(
            '<?xml version="1.0"?>' . "\n"
            . '<Request/>' . "\n",
