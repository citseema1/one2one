...
class DefaultCacheStorageTest extends \Guzzle\Tests\GuzzleTestCase
{
    protected function getCache()
    {
        ...
        $request = new Request('GET', 'https://foo.com', array('Accept' => 'application/json'));
    ...
    {
        $cache = $this->getCache();
        $this->assertNull($cache['storage']->fetch(new Request('GET', 'https://test.com')));
    }
    ...
    public function testPurgeRemovesAllMethodCaches()
    {
        ...
        $cache['storage']->purge('https://foo.com');
        $this->assertFalse(in_array('test', $this->readAttribute($cache['cache'], 'data')));
        $this->assertFalse(in_array($cache['serialized'], $this->readAttribute($cache['cache'], 'data')));
        $this->assertEquals(
    ...
    {
        $cache = $this->getCache();
        $request = new Request('GET', 'https://xyz.com');
    ...
    public function testUsesVaryToDetermineResult()
    {
        ...
        $request = new Request('GET', 'https://foo.com', array('Accept' => 'application/xml'));
        $this->assertNull($cache['storage']->fetch($request));
    }
    ...
    public function staleProvider()
    {
        return array(
            array(
                new Request('GET', 'https://foo.com', array('Accept' => 'foo')),
