...
class CurlMultiTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    {
        $multi = new CurlMulti();
        $request1 = new Request('GET', 'https://www.google.com/');
        $multi->add($request1);
        $this->assertEquals(array($request1), $multi->all());
        $request2 = new Request('POST', 'https://www.google.com/');
        $multi->add($request2);
    ...
    public function testRequestsCanBeRemoved()
    {
        ...
        $request2 = new Request('PUT', 'https://www.google.com/');
    ...
    {
        $this->multi->add(new Request('GET', 'https://www.google.com/'));
        $this->multi->reset();
    ...
    public function testCurlErrorsAreCaught()
    {
        ...
        try {
            $request = RequestFactory::getInstance()->create('GET', 'https://127.0.0.1:9876/');
            ...
            $this->assertContains('[url] https://127.0.0.1:9876/', $m);
    ...
    public function testDoesNotSendRequestsDecliningToBeSent()
    {
        ...
        $client = new Client('https://127.0.0.1:123', array(
    ...
    public function testDoesNotThrowExceptionsWhenRequestsRecoverWithSuccess()
    {
        ...
        $client = new Client('https://127.0.0.1:123');
        $request = $client->get();
