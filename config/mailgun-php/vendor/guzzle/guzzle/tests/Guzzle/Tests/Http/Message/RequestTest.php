...
class RequestTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testConstructorBuildsRequestWithArrayHeaders()
    {
        ...
        $request = new Request('GET', 'https://www.guzzle-project.com/', array(
            'foo' => 'bar'
        ));

        $this->assertEquals('GET', $request->getMethod());
        $this->assertEquals('https://www.guzzle-project.com/', $request->getUrl());
    ...
    {
        $request = new Request('GET', 'https://www.guzzle-project.com/', new Collection(array(
            'foo' => 'bar'
        )));
        $this->assertEquals('bar', $request->getHeader('foo'));
    }
    ...
    {
        $request = new Request('GET', 'https://www.guzzle-project.com/', null);
    ...
    public function testRequestsCanBeConvertedToRawMessageStrings()
    {
        ...
        $request = RequestFactory::getInstance()->create('PUT', 'https://www.google.com/path?q=1&v=2', array(
            'Authorization' => 'Basic ' . $auth
        ), 'Data');
    ...
    {
        $request = new Request('GET', 'https://0/0?0');
    ...
    {
        $url = 'https://www.test.com:8081/path?q=123#fragment';
        $u = Url::factory($url);
    ...
    {
        $this->request->setUrl('https://michael:test@test.com/');
        $this->assertEquals('michael', $this->request->getUsername());
    ...
    {
        $request = new Request('GET', 'https://www.example.com/foo?abc=d');
        $this->assertInstanceOf('Guzzle\Http\Url', $request->getUrl(true));
        $this->assertEquals('https://www.example.com/foo?abc=d', $request->getUrl());
        $this->assertEquals('https://www.example.com/foo?abc=d', (string) $request->getUrl(true));
    }
