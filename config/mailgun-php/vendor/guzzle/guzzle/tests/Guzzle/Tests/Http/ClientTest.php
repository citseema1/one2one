...
class ClientTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testAcceptsConfig()
    {
        ...
        $this->assertEquals('https://www.google.com/', $client->getBaseUrl());
        ...
        $this->assertSame($client, $client->setBaseUrl('https://www.test.com/{test}'));
        $this->assertEquals('https://www.test.com/123', $client->getBaseUrl());
        $this->assertEquals('https://www.test.com/{test}', $client->getBaseUrl(false));
    ...
    {
        $client = new Client('https://www.test.com/', array(
            'data' => '123'
        ));
    ...
    {
        $client = new Client('https://www.{foo}.{data}/', array(
            'data' => '123',
            'foo' => 'bar'
        ));
        $this->assertEquals('https://www.bar.123/', $client->getBaseUrl());
        $client->setBaseUrl('https://www.google.com/');
        $this->assertEquals('https://www.google.com/', $client->getBaseUrl());
    }
    ...
    public function testClientAddsParamsToRequests()
    {
        ...
        $client = new Client('https://www.example.com', array(
            'api' => 'v1',
    ...
    {
        $client = new Client('https://{a}/{b}');
        $this->assertEquals('https:///', $client->getBaseUrl());
        $this->assertEquals('https://{a}/{b}', $client->getBaseUrl(false));
        $client->setConfig(array(
            'a' => 'test.com',
            'b' => 'index.html'
        ));
        $this->assertEquals('https://test.com/index.html', $client->getBaseUrl());
    }
    ...
    public function testCreatesRequestsWithDefaultValues()
    {
        ...
        $request = $client->createRequest('HEAD', 'https://www.test.com/');
        $this->assertEquals('HEAD', $request->getMethod());
        $this->assertEquals('https://www.test.com/', $request->getUrl());
    ...
    {
        $client = new Client('https://www.test.com/api/v1', array(
            'test' => '123'
        ));
        $request = $client->get('relative/{test}');
        $this->assertEquals('https://www.test.com/api/v1/relative/123', $request->getUrl());
    }
    ...
    {
        $client = new Client('https://test.com', array(
            'path'  => array('foo', 'bar'),
            'query' => 'hi there',
            'data'  => array(
                'test' => 'a&b'
        ...
        $this->assertEquals('https://test.com/foo/bar?query=hi%20there&test=a%26b', $request->getUrl());
    ...
    public function testQueryStringsAreNotDoubleEncodedUsingAbsolutePaths()
    {
        ...
        $request = $client->get('https://test.com{?query}');
        $this->assertEquals('https://test.com?query=hi%20there', $request->getUrl());
        $this->assertEquals('hi there', $request->getQuery()->get('query'));
    }

    public function testAllowsUriTemplateInjection()
    {
        $client = new Client('https://test.com');
    ...
    {
        $client = new Client('https://test.com', array('test' => 'hi'));
        $ref = new \ReflectionMethod($client, 'expandTemplate');
        $ref->setAccessible(true);
        $uri = $ref->invoke($client, 'https://{test}{?query*}', array('query' => array('han' => 'solo')));
        $this->assertEquals('https://hi?han=solo', $uri);
    }
    ...
    {
        $client = new Client();
        $request = $client->createRequest('GET', 'https://www.foo.com', array('User-agent' => 'foo'));
    ...
    public function testCanSetDefaultRequestOptions()
    {
        ...
        $request = $client->createRequest('GET', 'https://www.foo.com?test=hello', array('Foo' => 'Test'));
    ...
    {
        $client = new Client();
        $request = $client->createRequest('GET', 'https://www.foo.com?test=hello', array('Foo' => 'Test'), null, array(
    ...
    {
        $client = new Client();
        $head = $client->head('https://www.foo.com', array(), array('query' => array('foo' => 'bar')));
        $this->assertEquals('bar', $head->getQuery()->get('foo'));
    }

    public function testCanSetRelativeUrlStartingWithHttp()
    {
        $client = new Client('https://www.foo.com');
        $this->assertEquals(
