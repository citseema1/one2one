...
templates. Guzzle supports the entire `URI templates RFC <https://tools.ietf.org/html/rfc6570>`_. URI templates add a
...
.. code-block:: php
    ...
    $request = $client->get(array('https://example.com{+path}{/segments*}{?query,data*}', array(
        'path'     => '/foo/bar',
...
The resulting URL would become ``https://example.com/foo/bar/one/two?query=test&more=value``.
...
`URI templates RFC <https://tools.ietf.org/html/rfc6570>`_.
