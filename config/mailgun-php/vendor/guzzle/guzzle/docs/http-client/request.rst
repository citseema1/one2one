...
`GET requests <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.3>`_ are the most common form of HTTP
...
.. code-block:: php
    ...
    $request = $client->get('https://www.amazon.com?a=1', array('X-Foo' => 'Bar'));
...
.. code-block:: php
    ...
    $request = $client->get('https://test.com', array(), array('save_to' => '/path/to/file'));

    // Send the response body to an fopen resource
    $request = $client->get('https://test.com', array(), array('save_to' => fopen('/path/to/file', 'w')));

HEAD requests
~~~~~~~~~~~~~

`HEAD requests <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.4>`_ work exactly like GET requests except
...
.. code-block:: php
    ...
    $request = $client->head('https://www.amazon.com');
    $response = $request->send();
...
A `DELETE method <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.7>`_ requests that the origin server
delete the resource identified by the Request-URI.

.. code-block:: php

    $client = new Guzzle\Http\Client();
    $request = $client->delete('https://example.com');
    $response = $request->send();

POST requests
~~~~~~~~~~~~~

While `POST requests <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.5>`_ can be used for a number of
...
.. code-block:: php
    ...
    $request = $client->post('https://httpbin.org/post', array(), array(
...
.. note::
    ...
    .. code-block:: php
        ...
        $request = $client->post('https://www.example.com', array(), array (
...
.. code-block:: php
    ...
    $request = $client->post('https://httpbin.org/post', array(), 'this is the body');
    $response = $request->send();
...
The `PUT method <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.6>`_ requests that the enclosed entity be
...
.. code-block:: php
    ...
    $request = $client->put('https://httpbin.org/put', array(), 'this is the body');
    $response = $request->send();
...
.. code-block:: php
    ...
    $request = $client->put('https://httpbin.org/put');
    $request->setBody(fopen('/path/to/large_body.txt', 'r'));
    $response = $request->send();
...
`PATCH requests <https://tools.ietf.org/html/rfc5789>`_ are used to modify a resource.

.. code-block:: php

    $request = $client->patch('https://httpbin.org', array(), 'this is the body');
    $response = $request->send();

OPTIONS requests
~~~~~~~~~~~~~~~~

The `OPTIONS method <https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.2>`_ represents a request for
information about the communication options available on the request/response chain identified by the Request-URI.

.. code-block:: php

    $request = $client->options('https://httpbin.org');
    $response = $request->send();
...
.. code-block:: php
    ...
    $request = $client->createRequest('COPY', 'https://example.com/foo', array(
...
.. code-block:: php
    ...
    $request = new Guzzle\Http\Message\Request('GET', 'https://www.example.com?foo=bar&abc=123');
...
.. code-block:: php
    ...
    $request = new Request('GET', 'https://httpbin.com/cookies');
...
If the Content-Length cannot be determined (i.e. using a PHP ``https://`` stream), then Guzzle will automatically add
the ``Transfer-Encoding: chunked`` header to the request.

.. code-block:: php

    $request = $client->put('/user.json');
    $request->setBody(fopen('https://httpbin.org/get', 'r'));
...
.. note::
    ...
    which causes cURL to sleep for `1 second <https://curl.haxx.se/mail/lib-2010-01/0182.html>`_.
...
.. code-block:: php
    ...
    $request = $this->client->get('https://example.com/large.mov', array(), array(
...
.. code-block:: php
    ...
    $request = $this->client->get('https://example.com/large.mov');
...
who need access to `cURL specific functionality <https://www.php.net/curl_setopt>`_ can still add cURL handle
...
.. code-block:: php
    ...
    $request = $client->get($url, $headers, array('proxy' => 'https://proxy.com'));
