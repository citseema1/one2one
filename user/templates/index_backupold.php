...
<?php
$select2_group  = mysqli_query($con,'select member_type from '.$prefix.'O2O_Member_Types order by priority asc');
while($fetch_group = mysqli_fetch_array($select2_group)){
	$get_member_type = $fetch_group['member_type'];


$select2  = mysqli_query($con,"select * from $prefix.O2O_Sponsors_Details where group_type='$get_member_type'");
$count=0; 
while($fetch2 = mysqli_fetch_array($select2)){

// echo $fetch2['website'];die;
?> 
<div class="col-1-3 tc pb  "  style="position: relative;" >
<!-- <a href="<?=$fetch2['website'] ?>" target="blank"><img  height="100px" src="<?php echo $fetch2['logo'];?>"> -->
<?php
$url = $fetch2['website'];

?>
<span class="grTypeName"><?php echo ucfirst($get_member_type);?></span>
<a href="<?php echo $url;?>" target="_blank">

<img  height="100px" src="<?php echo $fetch2['logo'];?>">
...
<div class="col-1-1">
<div class="content">
&copy; <?php echo $global_current_year = date('Y');  ?> Powered by one2onescheduler.com - All rights reserved 
</div>
</div>
</div>
</div>
...
//  TOAST METHODS--    success  ,  info	,    warning   ,  error
toastr[type](message);
}
//showToast("hello test","success");
</script>
<?php getMessage() ?>
...
<script src="templates/js/jquery.loadingModal.min.js"></script>
<script src="templates/js/customJs.js"></script>
<script>
/*$('body').loadingModal({text: 'Loader Please Wait...'}).loadingModal('animation', 'wave');
$(window).on('load', function() {
setTimeout(()=>{$('body').loadingModal('destroy') }, 300);
});*/
</script>
</body>
</html>